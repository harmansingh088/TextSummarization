...
public class PSO {
    ...
    private static void initialize() throws FileNotFoundException, IOException {
        ...
        String previous_file_read = "Internship\\review_old.txt";
        String previous_file_write = "Internship\\review_old1.txt";
        Info info = new Info();
        info.Info(previous_file_read, previous_file_write);
        Pre p = new Pre();
        p.Pre(s, previous_file_write);
        Vector score = new Vector(3, 2);
        FileReader fileReader = new FileReader("Internship\\Pso_lines_score.txt");
    ...
    private static void PSOAlgorithm() throws IOException {
        int gBest = 0;
        int gBestTest = 0;
    ...
    private static void printSolution() throws IOException {
        ...
        FileWriter fw = new FileWriter(username + "summary.txt");
        BufferedWriter bw = new BufferedWriter(fw);
        // Display elements

        for (int j = 0; j < MAX_INPUTS; j++) {

            System.out.print(((Vector) rt.get(i + 1)).get(j) + " ");
            bw.write("" + ((Vector) rt.get(i + 1)).get(j) + ". ");
